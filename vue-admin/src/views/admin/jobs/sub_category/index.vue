                                ...
                                <div class="card-body">
                                    <div class="card-header mb-2">
                                        <div class="row">
                                            ...
                                            <div class="col-md-1">
                                                <button type="button" class="btn btn-primary waves-effect waves-light" data-toggle="modal" v-if="this.hasActionPermission(4,'create')" data-target=".addModal">Add</button>
                                            </div>
                                        </div>
                                        <div >
                                        </div>
                                    ...
                                    <table id="datatable" class="table table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                                        ...
                                        <thead>
                                            <tr>
                                                <th></th>
                                                    <th>
                                                        <input class="form-control" type="text" v-model="subCategorySearch" placeholder="Search by Sub Category...">
                                                    </th>
                                                    <th>
                                            ...
                                            <tr v-for="(item,index) in filteredResult" :key="index">
                                                ...
                                                <td>
                                                    <center>
                                                        ...
                                                        &nbsp
                                                        <button v-if="this.hasActionPermission(4,'delete')" class="btn btn-danger btn-sm" @click="delete_cat(item.id)"><i class="bx bx-bx bx-trash"></i></button></center>
                                                </td>
                                            </tr>
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title mt-0" id="myLargeModalLabel">Add Sub Category</h5>
                ...
                <div class="modal-body">
                    <form @submit.prevent="onSubmit">
                        <div class="form-group row">
                            ...
                            <div class="col-md-3">
                                <input class="form-control" type="number" required v-model="form.price" >
...
<div class="modal fade updateModal" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title mt-0" id="myLargeModalLabel">Update Sub Category</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <form @submit.prevent="onSubmit">
                        ...
                        <div class="form-group row">
                                <label class="col-md-2 col-form-label">Category</label>
                                <div class="col-md-10">
                                    <select class="form-control" v-model="v$.updateForm.category.$model">
                                        <option selected>Select</option>
                                        <option v-for="option in categories" :key="option.id" :value="option.id">
                                            {{ option.name }}
                                        </option>
...
<script>
    ...
    export default {
        ...
        created() {
                this.getResult();
                this.getcategories();
                this.hasModulePermission(4);
            },

            computed: {
                            filteredResult() {
                                    const filteredCats = this.result.filter(cat => {
                                        const nameMatch = cat.name.toLowerCase().includes(this.subCategorySearch.toLowerCase());
                                        const titleMatch = cat.category.toLowerCase().includes(this.categorySearch.toLowerCase());
        ...
        methods: {
                ...
                async getResult(page=1) {
                        ...
                        const data =  await this.api('GET',this.$service+'all_sub_category',this.form,false,false)
                        if(data.status===200){
                ...
                async getcategories(page=1) {
                        ...
                        const data =  await this.api('GET',this.$service+'category',this.form,false,false)
                ...
                async save() {
                            ...
                            const data =  await this.api('POST',this.$service+'save_sub_category',formData,false,true)
                            if(data.status===200){
                ...
                async fetchInfo(id) {
                        ...
                        const data = await this.api('POST',this.$service+'singleSubCategory',{id:id},false,false)
                        if(data.status===200){
                            $('.updateModal').modal('show');
                            this.updateForm.id = data.data.id;
                            this.updateForm.name = data.data.name;
                            this.updateForm.price = data.data.price;
                            this.updateForm.category = data.data.category_id;
                ...
                async updateSave(page=1) {
                    ...
                    const data =  await this.api('POST',this.$service+'updateSubCategory',formData,false,true)
                ...
                async  delete_cat(id) {
                    ...
                    const data =  await this.api('POST',this.$service+'delete_sub_category',data1,false,true)
