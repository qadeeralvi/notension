                                ...
                                <div class="card-body">
                                    ...
                                    <div class="card-header mb-2">
                                        <div class="row">
                                            ...
                                            <div class="col-md-1">
                                                <button type="button" v-if="this.hasActionPermission(8,'create')" class="btn btn-primary waves-effect waves-light" @click="addModal()">Add</button>
                                            </div>
                                        </div>
                                    </div>
                                        ...
                                        <tbody>
                                            ...
                                            <tr v-for="(item,index) in filteredResult" :key="index">
                                                ...
                                                <td style="display:flex; justify-content: space-around; align-items: center;">
                                                    <button style="margin-right: 2px;" class="btn btn-warning btn-sm" v-if="this.hasActionPermission(8,'update')" @click="fetchInfo(item.id)"><i class="bx bx-edit"></i></button>
                                                    <button style="margin-left: 2px;" class="btn btn-danger btn-sm"  v-if="this.hasActionPermission(8,'delete')" @click="delete_id(item.id)"><i class="bx bx-bx bx-trash"></i></button>
                                                </td>
                                            </tr>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form @submit.prevent="onSubmit">
                        ...
                        <div class="form-group row" v-if="showDay">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form @submit.prevent="onSubmit">
                        ...
                        <input type="hidden" v-model="updateForm.providerId">
                        <div class="form-group row">
                            <label for="example-text-input" class="col-md-2 col-form-label">Name</label>
                            <div class="col-md-10">
                        ...
                        <div class="form-group row">
                            ...
                            <label for="example-text-input" class="col-md-2 col-form-label">Phone</label>
                            <div class="col-md-10">
                                <input class="form-control phone" type="text" required v-model="v$.updateForm.phone_no.$model" placeholder="3000000000">
    ...
    export default {
        ...
        methods: {
                async getResult(page=1) {
                        const data =  await this.api('POST',this.$authentication+'provider_list',this.form,false,false)
                        if(data.status===200){
                            this.result =  data.data
                            this.form.status =  this.result.status
                        }
                },

                async save() {
                    const data =  await this.api('POST',this.$authentication+'service-provider/signup',this.form,false,true)
                    if(data.status===200){
                        setTimeout(() => {
                            location.reload();
                        }, 2000);
                ...
                async saveStatus(page=1) {
                        const data =  await this.api('POST',this.$authentication+'change_provider_status',{status:this.provider_status,provider_id:this.provider_id},true,true)
                        if(data.status===200){
                            location.reload();
                        }
                },
                ...
                async updateSave(page=1) {
                    ...
                    const data =  await this.api('POST',this.$authentication+'provider_update',{
                            provider_id:this.updateForm.providerId,
                ...
                async fetchInfo(id) {
                        const data = await this.api('POST',this.$authentication+'provider_info',{id:id},false,false)
                        if(data.status===200){
                            $('.updateModal').modal('show');
                            this.updateForm.providerId = data.provider.id;
                            this.updateForm.name = data.provider.name;
                            this.updateForm.email = data.provider.email;
                            this.updateForm.phone_no = data.provider.phone_no;
                            this.updateForm.location = data.provider.location;
                        }
                },
                ...
                statusValue(){
                    if(this.provider_status=='suspend'){
                        this.showDay = true;
                    }
                    else{
                        this.showDay = false;
                ...
                exportToPDF() {
                        ...
                        const filteredData = this.filteredResult;
                        const pdf = new jsPDF();
                        const columns = ['Name', 'Email', 'Signup Method', 'Phone Number','Location'];
                        const data = filteredData.map(item => [item.name, item.email, item.signupmethod,item.phone_no,item.location]);
